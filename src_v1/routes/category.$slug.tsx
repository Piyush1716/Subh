...
function CategoryPage() {
  ...
  const sorted = [...products].sort((a, b) => {
    ...
    return 0; // default: newest first (already sorted by Supabase)
  });
      ...
      <main className="flex-1">
        ...
        <div className="bg-secondary/60 border-b border-border">
          <div className="max-w-7xl mx-auto px-4 lg:px-6 py-8 sm:py-12">
            ...
            <p className="mt-3 max-w-2xl text-sm sm:text-base text-muted-foreground">{category.description}</p>
          </div>
        </div>
                ...
                {categories.map((c) => (
                  <li key={c.slug}>
                    <Link
                      to="/category/$slug"
                      params={{ slug: c.slug }}
                      className={`flex justify-between hover:text-primary transition-colors ${c.slug === category.slug ? "text-primary font-medium" : ""}`}
                    >
                      <span>{c.name}</span>
                    </Link>
                  </li>
